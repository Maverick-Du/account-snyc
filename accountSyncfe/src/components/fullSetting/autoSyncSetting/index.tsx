...
export default function AutoSyncSetting() {
  ...
  const changeFullOpen = async (checked: boolean) => {
    const res = await fullSyncApi.setFullSyncSchedule({ open: Number(checked) });
  ...
  return (
    <>
      ...
      <div className={style["setting-item"]}>
        ...
        <Switch checked={!!open} onChange={changeFullOpen} />
      </div>
      <div className={style["setting-tip"]}>{ZH_CN["sync_auto_status_setting_full_tips"]}</div>
    </>
  );
}
