...
root.post('/api/manage/fullsync/task/stop', async (ctx: Context) => {
  let logTaskId = ''
  try {
    ...
    await admin.stopFullSyncTask(taskId, companyId, userId, userName)
    ctx.body = new Result(Result.SUCCESS_CODE, 'success')
    let detail = OPERATION_MAP.get(AdminOperationTypeEnum.STOP_FULL_SYNC_TASK).detailCallback(taskId, true)
    logService.logSuccess(ctx, AdminOperationTypeEnum.STOP_FULL_SYNC_TASK, detail)
  } catch (err) {
    log.i(err);
    let detail = OPERATION_MAP.get(AdminOperationTypeEnum.STOP_FULL_SYNC_TASK).detailCallback(logTaskId, false)
    logService.logError(ctx, AdminOperationTypeEnum.STOP_FULL_SYNC_TASK, err, detail)
