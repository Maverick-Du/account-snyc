...
root.get('/api/manage/fullsync/detail/download', async (ctx: Context) => {
  let logTaskId = ''
  try {
    ...
    const workbook = await admin.fullSyncDetailDownload(companyId, taskId)
    ctx.set('Content-Type', 'application/octet-stream;charset=utf-8')
    ctx.set('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(`【账号对接】全量同步数据详情-${taskId}`)}.xlsx`)
    ctx.status = 200
    ...
    let detail = OPERATION_MAP.get(AdminOperationTypeEnum.DOWNLOAD_FULL_SYNC_DETAIL).detailCallback(logTaskId, 'normal', true)
    logService.logSuccess(ctx, AdminOperationTypeEnum.DOWNLOAD_FULL_SYNC_DETAIL, detail)
    return ctx
  } catch (err) {
    log.i(err);
    let detail = OPERATION_MAP.get(AdminOperationTypeEnum.DOWNLOAD_FULL_SYNC_DETAIL).detailCallback(logTaskId, 'normal', false)
