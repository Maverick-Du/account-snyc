...
import {AnalyseListOvs, FullSyncUpdateType, RecordStatus, StatisticAnalyseOperateType} from '../types'

export interface FullSyncUserRecord {
  id: number,
  task_id: string,
  company_id: string,
  name: string,
...
export class FullSyncUserRecordTable extends Table<FullSyncUserRecord> {
  constructor(db: IDatabase) {
    super(db, 'tb_full_sync_user_record')
  }

  async addRecord(ovs: Partial<FullSyncUserRecord>): Promise<number> {
    return this.add(ovs).query()
  }

  async addRecords(ovs: Partial<FullSyncUserRecord>[]) {
    return this.db.insert('insert into tb_full_sync_user_record (task_id, company_id, `name`, account, platform_id, uid, wps_did, abs_path, update_type, `status`, msg, err_type) values ?', [ovs.map(o => [o.task_id, o.company_id, o.name, o.account, o.platform_id, o.uid, o.wps_did, o.abs_path, o.update_type, o.status, o.msg, o.err_type])])
  }

  async countData() {
    return this.db.select('select count(1) as total from tb_full_sync_user_record', [])
  }
  ...
  async countUserAddErr(companyId: string, taskId: string) {
    const countList: any[] = await this.select(`count(1) as total`).where(`task_id = ? and company_id = ? and status = ? and update_type = ?`, taskId, companyId, RecordStatus.FAIL, FullSyncUpdateType.UserAdd).query()
  ...
  async getAnalyseRecordsByStatusAndType(taskId: string, status: RecordStatus, updateType: FullSyncUpdateType) {
    return this.select('platform_id,uid').where('task_id = ? and status = ? and update_type = ?', taskId, status, updateType).query()
  }
