...
export class IncrementDeptAddStrategy implements Strategy<IncrementSyncContext, StrategyResult> {
    name: string = IncrementStrategyType.DeptAdd
    ...
    async exec(ctx: IncrementSyncContext): Promise<StrategyResult> {
        ...
        log.i({ info: `increment sync deptAdd companyId: ${ctx.cfg.companyId}, did: ${dept.did}, pid: ${dept.pid}, name: ${dept.name}, source: ${dept.source}, order: ${dept.order} start`})
        try {
            ...
            if (!data.exist) {
                ...
                if (dept.name !== exist.name || order !== exist.order) {
                    ...
                    log.i({ info: `increment sync deptAdd updateDepartment success. companyId: ${ctx.cfg.companyId}, did: ${dept.did}`})
                }
                // move
                parent = data.parent
                if (exist.dept_pid != parent.dept_id) {
                    ...
                    log.i({ info: `increment sync deptAdd moveDepartment success. companyId: ${ctx.cfg.companyId}, did: ${dept.did}`})
                }
            }

            await las.updateDeptSyncData(dept.id, IncrementStatus.SUCCESS, data.msg)
            log.i({ info: `increment sync deptAdd success. companyId: ${ctx.cfg.companyId}, did: ${dept.did} end`})
        } catch (err) {
            let flag = await this.handleDeptNameExists(err, parent, dept)
            if (flag) {
                await las.updateDeptSyncData(dept.id, IncrementStatus.SUCCESS, "success")
            } else {
                err.msg = `increment sync deptAdd throw error. msg: ${err.message}, dept: ${JSON.stringify(dept)}`
                log.e(err)
                ctx.statistics.dept_fail++
    ...
    async handleDeptNameExists(err: any, parent: WPSDepartment, dept: LasDeptIncrementSchema) {
        try {
            ...
            if (resCode == WpsApiErrorCode.DeptNameExists) {
                if (config.strategy.dept_name_conflict == IncrementSyncDeptNameConflict.RENAME) {
                    ...
                    await sync.ctx.engine.was.addDepartment(parent.company_id, parent.dept_id, dept.platform_id, dept.did, name,  dept.order || 0, dept.source, dept.type)
                    log.i({ info: `increment sync deptAdd handleDeptNameExists success. deptName: ${name}, deptId: ${dept.did}` })
                    return true
                }
            }
            return false
        } catch (er) {
            er.msg = `increment sync deptAdd handleDeptNameExists throw error. msg: ${er.message}, dept: ${JSON.stringify(dept)}`
            log.e(er)
            return false
        }
    }
    ...
    async checkDeptData(ctx: IncrementSyncContext, dept: LasDeptIncrementSchema): Promise<CheckResponseData> {
        try {
            ...
            if (!dept.pid || !dept.name) {
                return {
                    flag: false,
                    msg: `缺失pid/name等关键参数无法创建部门`
                }
            }
            let parent = null
            ...
            err.msg = `increment sync deptAdd handleDeptData throw error. msg: ${err.message}, dept: ${JSON.stringify(dept)}`
            log.e(err)
            return {
                flag: false,
                msg: err.message
