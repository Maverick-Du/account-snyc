 ...
 *
 */
export class MoveDepartmentTreeStrategy implements IMoveDepartmentTreeStrategy {
  name: string = SyncStrategyType.MoveDepartmentTree

  async exec(ctx: IMoveDepartmentTreeContext): Promise<IMoveDepartmentTreeResult> {
    const { engine, task, parent, dept, from } = ctx
    const tick = new Ticker()
    log.debug({ info: `full sync ${task.taskId} moveDeptTree deptId: ${dept.dept_id} deptName: ${dept.name} parentId: ${parent?.dept_id} start` })
    // 1. 先挪动树
    if (dept.dept_pid != parent?.dept_id) {
      await this.moveDepartment(engine, task, parent, dept)
      log.debug({ info: `full sync ${task.taskId} moveDeptTree deptId: ${dept.dept_id} deptName: ${dept.name} parentId: ${parent?.dept_id} success[${tick.end()}]` })
    }

    // 2. 再更新属性
    await this.updateDepartment(engine, task, dept, from)
    log.debug({ info: `full sync ${task.taskId} moveDeptTree.updateDepartment deptId: ${dept.dept_id} deptName: ${dept.name} fromId: ${from.did} success[${tick.end()}]` })

    // 3. 同步子目录
    await this.syncChildren(engine, task, dept, from)
    log.debug({ info: `full sync ${task.taskId} moveDeptTree.syncChildren deptId: ${dept.dept_id} deptName: ${dept.name} fromId: ${from.did} success[${tick.end()}]` })
  ...
  async updateDepartment(
    ...
    const ctx: IUpdateDepartmentContext = {
      engine, dept, from, task
    }
    await engine.sm.exec(SyncStrategyType.UpdateDepartment, ctx)
  }
  ...
  async syncChildren(
    ...
    for (const add of Object.values(tmps)) {
      task.push(new AddDepartmentTreeAction(dept, add))
    }
